...
$wallpapersDir = if ($env:LANG -match "tr") { "$env:USERPROFILE\Resimler\Wallpapers" } else { "$env:USERPROFILE\Pictures\Wallpapers" }
...
try {
    ...
    git clone --depth 1 https://github.com/Venxe/wallpapers.git $installDir | Write-Host
